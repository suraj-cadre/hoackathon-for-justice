...
class Contract(Base):
    ...
    status = Column(Enum(ContractStatus, values_callable=lambda x: [e.value for e in x]), default=ContractStatus.UPLOADED, nullable=False, index=True)
...
class AnalysisResult(Base):
    ...
    findings = relationship("DisputeFinding", back_populates="analysis_result", cascade="all, delete-orphan")
...
class DisputeFinding(Base):
    ...
    issue_type = Column(Enum(IssueType, values_callable=lambda x: [e.value for e in x]), nullable=False, index=True)
    severity = Column(Enum(Severity, values_callable=lambda x: [e.value for e in x]), nullable=False)
